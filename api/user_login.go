...
var (
	getConsoleCredentialsForLogin     = getConsoleCredentials
	getLDAPConsoleCredentialsForLogin = getLDAPConsoleCredentials
...
// getLDAPConsoleCredentials returns ConsoleCredentials backed by LDAP-issued STS credentials.
func getLDAPConsoleCredentials(accessKey, secretKey string, client *http.Client) (*ConsoleCredentials, error) {
	creds, err := auth.GetCredentialsFromLDAP(getMinIOServer(), accessKey, secretKey)
	if err != nil {
		return nil, err
	}
	return &ConsoleCredentials{
